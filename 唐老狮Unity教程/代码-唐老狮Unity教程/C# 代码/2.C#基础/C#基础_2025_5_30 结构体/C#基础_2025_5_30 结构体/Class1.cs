...
namespace Lesson12_练习题
{
    ...
    struct Boss
    {
        ...
        public void Atk(ref Ultraman outMan)
        {
            //小怪兽打奥特曼的逻辑
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Ultraman ultraman = new Ultraman("迪迦奥特曼", 10, 5, 100);
            Boss boss = new Boss("章鱼怪", 8, 4, 100);

            while (true)
            {
                ultraman.Atk(ref boss);
                if (boss.hp <= 0)
                {
                    Console.WriteLine("{0}胜利", ultraman.name);
                    break;
                }
                boss.Atk(ref ultraman);
                if (ultraman.hp <= 0)
                {
            ...
            Student s1 = new Student("David", false, 27, 1, "政治学");
            s1.Speak();
            ...
            string userInput = Console.ReadLine(); // 先读取用户输入

            // 1. 尝试转换成数字
            if (int.TryParse(userInput, out int input))
            {
                // 2. 检查数字是否在枚举范围内
                if (Enum.IsDefined(typeof(E_Occupation), input))
                {
                    E_Occupation o = (E_Occupation)input; // 转换枚举
                    PlayerInfo info = new PlayerInfo(name, o);
                    info.Atk();
                }
                else
                {
                    Console.WriteLine("请输入正确的数字（0、1、2）");
                }
            }
            else
            {
                Console.WriteLine("请输入数字（0、1、2）");
            }
        }
    }
}
