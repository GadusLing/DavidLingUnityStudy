...
/// <summary>
/// ExcelReader 读取 Excel → DataSet → 转 JSON
/// </summary>
public class ExcelUtility
{

	/// <summary>
	/// 表格数据集合     DataSet 理解成：一本 Excel 文件   DataTable 理解成：Excel 里的每一个 Sheet 页
	/// </summary>
	...
	public ExcelUtility(string excelFile)
	{
		...
		IExcelDataReader mExcelReader = ExcelReaderFactory.CreateOpenXmlReader(mStream); //用 excelDataReader 读取 xlsx
		mResultSet = mExcelReader.AsDataSet();// ，转成 DataSet
	}
	...
	public void ConvertToJson(string JsonPath, Encoding encoding)
	{
		...
		for (int x = 0; x < mResultSet.Tables.Count; x++)
		{
			...
			{
				Debug.Log("无法识别导出名 " + outname + "  请确定#书写正确!");
				continue;
			}
