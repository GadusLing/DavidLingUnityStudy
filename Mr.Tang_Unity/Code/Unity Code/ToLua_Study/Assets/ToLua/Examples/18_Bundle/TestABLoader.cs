...
public class TestABLoader : MonoBehaviour 
{
    ...
    IEnumerator CoLoadBundle(string name, string path)
    {
        using (UnityWebRequest uwr = UnityWebRequestAssetBundle.GetAssetBundle(path))
        {
            ...
            {
                Debugger.LogError(string.Format("Read {0} failed: {1}", path, uwr.error));
                yield break;
            }
    ...
    public IEnumerator LoadBundles()
    {
        ...
        using (UnityWebRequest uwr = UnityWebRequestAssetBundle.GetAssetBundle(main))
        {
            yield return uwr.SendWebRequest();

            if (uwr.result != UnityWebRequest.Result.Success)
            {
                Debugger.LogError(string.Format("Read manifest {0} failed: {1}", main, uwr.error));
                yield break;
            }
