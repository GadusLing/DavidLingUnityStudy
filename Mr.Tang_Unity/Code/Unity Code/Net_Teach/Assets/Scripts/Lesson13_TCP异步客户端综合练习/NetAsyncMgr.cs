...
public class NetAsyncMgr : MonoBehaviour
{
    ...
    private Queue<BaseMsg> receiveQueue = new Queue<BaseMsg>(); // 消息队列
    ...
    void Update()
    {
        ...
        {
            BaseMsg baseMsg = receiveQueue.Dequeue(); // 取出一条消息
            switch (baseMsg)
            {
                case PlayerMsg msg:
                    Debug.Log(msg.playerID); // 打印玩家ID
                    Debug.Log(msg.data.name); // 打印玩家名字
                    Debug.Log(msg.data.lev); // 打印玩家等级
                    Debug.Log(msg.data.atk); // 打印玩家攻击力
                    break;
                // 可根据需要扩展更多消息类型
            }
        }
    ...
    // 处理接收到的消息，协议解析
    // 这里老师的接收消息代码有问题，服务器没有广播消息测试过，可以去看服务器里的TcpAsyncExercise_Server的ClientSocket的HandleReceiveMsg
    // 那个是用unity的Lesson13测试过的
    private void HandleReceiveMsg(int receiveNum) 
    {
        ...
        while (true)
        {
            ...
            if (cacheNum - nowIndex >= msgLength && msgLength != -1) // 缓冲区剩余字节足够一条完整消息
            {
                ...
                if (baseMsg != null)
                    receiveQueue.Enqueue(baseMsg); // 入队等待处理
