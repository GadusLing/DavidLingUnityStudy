...
public class Lesson31 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //1.二进制字节数组
        dataList.Add(new MultipartFormDataSection(Encoding.UTF8.GetBytes("123123123123123")));// 用的比较少
        //2.字符串
        dataList.Add(new MultipartFormDataSection("12312312312312312dsfasdf"));
        //3.参数名，参数值（字节数组，字符串），编码类型，资源类型（常用）
        dataList.Add(new MultipartFormDataSection("Name", "David", Encoding.UTF8, "application/...."));
    ...
    IEnumerator Upload()
    {
        ...
        UnityWebRequest req = UnityWebRequest.Post("http://192.168.1.2:8081/Http_Server/", data);
    ...
    {
        UnityWebRequest req = UnityWebRequest.Put("http://192.168.1.2:8081/Http_Server/", File.ReadAllBytes(Application.streamingAssetsPath + "/test.png"));
        ...
        {
            print("Put 上传失败" + req.error + req.responseCode + req.result);
        }
    }
