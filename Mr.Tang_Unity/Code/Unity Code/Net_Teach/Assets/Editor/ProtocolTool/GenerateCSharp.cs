...
public class GenerateCSharp
{
    ...
    private string GetFieldsStr(XmlNodeList fieldNodes)
    {
        ...
        foreach (XmlNode dataField in fieldNodes)
        {
            ...
            else if (fieldType == "dic")
            {
                ...
                defaultValueStr = $" = new Dictionary<{Tkey}, {Tvalue}>()"; // Dictionary 默认补空字典，避免外部直接 Add 时空引用
            }
            ...
            else if (!IsSimpleFieldType(fieldType))
            {
                defaultValueStr = $" = new {fieldType}()"; // 非基础类型/非枚举字段按数据结构类处理，默认创建实例，避免 msg.data.xxx 这类访问直接空引用
            }
            fieldStr += $"\t\tpublic {fieldType} {fieldName}{defaultValueStr};\r\n"; // 拼接类字段字符串，格式为 "public 变量类型 变量名 = 默认值;\r\n"
    ...
    /// <summary>
    /// 判断字段类型名是否属于基础简单类型。
    /// 这些类型默认值已经足够安全，不需要在生成代码时额外 new 实例。
    /// </summary>
    private bool IsSimpleFieldType(string fieldType)
    {
    ...
    /// <summary>
    /// 生成 GetID() 方法字符串，格式为 "public override int GetID() { return 消息ID; }" 消息ID 从配置文件中读取，每个消息类都有一个独特的消息ID，这个ID是在配置文件中定义的，BaseMsg类无法预知，所以需要在生成的消息类中重写GetID方法来实现这个功能，确保每个消息类都能正确返回它对应的消息ID，以便在协议通信中正确识别和处理不同类型的消息。
    /// </summary>
    /// <param name="id"></param>
    /// <returns></returns>
